      ...
      <Popover className="relative">
        {({ open }) => (
          <>
            ...
            >
              <Popover.Panel className="absolute left-1/2 z-10 mt-5 flex w-screen sm:mx-auto max-sm:-left-28 max-md:-left-28 -translate-x-1/2 px-4  mr-8 lg:max-w-3xl backdrop:blur-md">
                <div className="overflow-hidden rounded-br-lg rounded-bl-lg shadow-lg ring-1 ring-black ring-opacity-5">
                    <div className="relative grid gap-8 bg-white p-8 lg:grid-cols-2 backdrop:blur-md opacity-100">
                        {menu.map((item) => (
                            <a
                                key={item.name}
                                href={item.href}
